...
class PdfDownloader:
    ...
    def save_if_complete(self, temp_path: Path, article: ArticleRecord) -> ArticleRecord:
        ...
        if final_path.exists():
            ...
            return article
        self._move_with_retry(temp_path, final_path)
        article.local_pdf_path = final_path
    ...
    def download(self, article: ArticleRecord) -> ArticleRecord:
        ...
        if doi_path and doi_path.exists():
            ...
            self._move_with_retry(doi_path, final_path)
            article.local_pdf_path = final_path
        ...
        if semantic_variant and semantic_variant.exists():
            final_path.parent.mkdir(parents=True, exist_ok=True)
            self._move_with_retry(semantic_variant, final_path)
            article.local_pdf_path = final_path
            article.download_status = "skipped_existing_renamed"
            article.pdf_status = "complete"
            article.error = ""
    ...
    def _move_with_retry(self, source: Path, destination: Path) -> None:
        """Move a file, tolerating short-lived Windows file locks."""
        source = Path(source)
        destination = Path(destination)
        last_error = None
        for attempt in range(self.file_operation_retries + 1):
            try:
                shutil.move(str(source), str(destination))
                return
            except PermissionError as exc:
                last_error = exc
                if attempt >= self.file_operation_retries:
                    break
                time.sleep(self.file_operation_retry_delay * (attempt + 1))
        raise last_error

    def _safe_unlink(self, path: Path) -> None:
        """Best-effort cleanup that does not mask the download outcome."""
        path = Path(path)
        for attempt in range(self.file_operation_retries + 1):
            try:
